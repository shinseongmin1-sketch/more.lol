        ...
        <div className="community-layout">
          <div className="community-main">
            <div className="community-tabs">
              {categories.map((c, i) => (
                <button key={c} className={`community-tab ${i === 0 ? 'active' : ''}`}>{c}</button>
              ))}
            </div>
            ...
            <div className="post-list">
              {mockPosts.map(post => (
                <div key={post.id} className="post-row">
                  <span className="post-cat" style={{ color: catColors[post.cat] || 'var(--text-muted)', background: (catColors[post.cat] || 'var(--text-muted)') + '15' }}>
                    {post.cat}
                  </span>
